...
using JTensor
 ...
 """
 function square_peps_HeisenbergII(T,chi;maxiter=20,err=1e-8)
     d,D=size(T,1,2)

     Al=permutedims(reshape(qr(rand(Complex128,chi*D^2,chi))[1],chi,D,D,chi),[1,4,2,3])
     Ar=permutedims(Al,[2,1,3,4])
     Bl=Al
     Br=Ar
     Ac=CA=FlA=FrA=Bc=CB=FlB=FrB=[]

     L=rand(Complex128,chi,D,D,chi)
     R=rand(Complex128,chi,D,D,chi)
     ...
     for iter=1:maxiter
         Al,Ar,Ac,CA,FlA,FrA,feA,Aerr_mean=dl_one_vumps(permutedims(T,[1,2,4,3,5]),chi,Al,Ar,Ac,CA,FlA,FrA,maxiter=1,e0=Aerr_mean)
         Bl,Br,Bc,CB,FlB,FrB,feB,Berr_mean=dl_one_vumps(permutedims(T,[1,4,2,5,3]),chi,Bl,Br,Bc,CB,FlB,FrB,maxiter=1,e0=Berr_mean)
         ...
         λl=λl[1]
         λr=λr[1]
         L=reshape(L,chi,D,D,chi)
         R=reshape(R,chi,D,D,chi)
...
we print energy for every iteration
"""
 function square_peps_duc_HeisenbergII(TA,TB,chi;maxiter=20,err=1e-8)
     d,D=size(TA,1,2)

     Alu=Blu=Ald=Bld=permutedims(reshape(qr(rand(Complex128,chi*D^2,chi))[1],chi,D,D,chi),[1,4,2,3])
     ...
     for iter=1:maxiter
         Alu,Aru,Acu,Blu,Bru,Bcu,C1u,C2u,FAlu,FAru,FBlu,FBru,ufe,uerr_mean=dl_two_vumps(permutedims(TA,[1,2,4,3,5]),permutedims(TB,[1,2,4,3,5]),chi,Alu,Aru,Blu,Bru,Acu,Bcu,C1u,C2u,FAlu,FAru,FBlu,FBru,e0=uerr_mean,maxiter=1)
         #Ald,Ard,Acd,Bld,Brd,Bcd,C1d,C2d,FAld,FArd,FBld,FBrd,dfe,derr_mean=dl_two_vumps(permutedims(TA,[1,4,2,5,3]),permutedims(TB,[1,4,2,5,3]),chi,Ald,Ard,Bld,Brd,Acd,Bcd,C1d,C2d,FAld,FArd,FBld,FBrd,e0=derr_mean,maxiter=1)
