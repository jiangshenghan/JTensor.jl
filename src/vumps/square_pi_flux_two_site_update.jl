...
T_spin_rep are for spin rep for a virtual leg of single layer PEPS tensor T

returns(Al,Ar,chi+dchi,vals_spin_rep)
"""
function square_pi_flux_spin_sym_two_site_update(TT,Fl,Fr,dchi,T_spin_rep,chi_spin_rep; A2c=[],cut_ratio=0.8,e0=1e-12,elemtype=Complex128,ncv=20)
    chi=size(Al,1)
    DD=size(TT[1],1)
    D=Int(sqrt(DD))
    N=2
    ...
    Us,Ss,Vts,vals_spin_rep=svd_spin_sym_tensor(A2c,[1,2,3],[chi_spin_rep,T_spin_rep,T_spin_rep,chi_spin_rep,T_spin_rep,T_spin_rep],[1,-1,-1,1,-1,1])
    svals=vcat(Ss...)
