...
function dl_mult_vumps_seq(T,chi,Al=[],Ar=[],Ac=[],C=[],Fl=[],Fr=[];ep=1e-12,e0=1e-1,maxiter=50,elemtype=Complex128)
    ...
    for iter=1:maxiter
        ...
        for ns=1:N
            ...
            #left fixed point
            left_tensor_list=[Fl[ns]]
            left_legs_list=[[1,2,3,4]]
            for il=ns:(ns+N-1) 
                jl=(il-1)%N+1
            ...
            leig_res=eigs(leftlm,nev=1,v0=Fl[ns][:],tol=max(ep/100,err_mean/200,1e-15))
            λl,vl=leig_res
            λl=λl[1]
            ...
            #right fixed point
            right_tensor_list=[Fr[ns]]
            right_legs_list=[[1,2,3,4]]
            for ir=ns:-1:(ns-N+1) 
                jr=(ir+N-1)%N+1
            ...
            reig_res=eigs(rightlm,nev=1,v0=Fr[ns][:],tol=max(ep/100,err_mean/200,1e-15))
            λr,vr=reig_res
            λr=λr[1]
            ...
            Aceig_res=eigs(Aclm,nev=1,v0=Ac[ns][:],tol=max(ep/100,err_mean/200,1e-15))
            λAc,vAc=Aceig_res
            λAc=λAc[1]
            ...
            CLeig_res=eigs(CLlm,nev=1,v0=C[nsl][:],tol=max(ep/100,err_mean/200,1e-15))
            λCL,vCL=Ceig_res
            λCL=λCL[1]
            ...
            CReig_res=eigs(CRlm,nev=1,v0=C[ns][:],tol=max(ep/100,err_mean/200,1e-15))
            λCR,vCR=CReigs_res
            λCR=λCR[1]
            ...
            UAc,PAc=polardecomp(reshape(permutedims(Ac,[2,3,4,1]),chi*Dv^2,chi))
            ...
            err_Ar=vecnorm(Ac[ns]-jcontract([C[nsl],Ar],[[-1,1],[1,-2,-3,-4]]))
            err_max=max(err_max,err_Al,err_Ar)

            free_energy=mean([λr,λl])

            @printf("iteration %d, site %d \n free energy \n λl: %.16f + i %e \n λr: %.16f + i %e  \n error in prediction \n err_Fl: %.16e \n err_Fr: %.16e \n err_Ac: %.16e \n err_CL: %.16e \n err_CR: %.16e \n err_Al: %.16e \n err_Ar %.16e \n err_max %.16e \n \n",iter,ns,real(λl),imag(λl),real(λr),imag(λr),err_Fl,err_Fr,err_Ac,err_CL,err_CR,err_Al,err_Ar,err_max)
            flush(STDOUT)
        end

        if err_max<ep break end
    end
