...
function dl_mult_mpo_mps(A,T,chi,Fl=[],Fr=[];ep=1e-8,elemtype=Complex128,ncv=40)
    ...
    DA=size(A[1],1)
    @printf("chi=%d, ep=%e \n N=%d, Dh=%d, Dv=%d",chi,ep,N,d,Dh,Dv)

    Tc=conj(T)
    if Fl==[] Fl=[rand(elemtype,DA,Dh,Dh,Dh,Dh,DA) for i=1:N] end
    if Fr==[] Fr=[rand(elemtype,DA,Dh,Dh,Dh,Dh,DA) for i=1:N] end

    #left fixed point
    left_tensor_list=[]
    push!(left_tensor_list,Fl)
    for il=1:N
        append!(left_tensor_list,[A[il],T[il],Tc[il],T[il],Tc[il],conj(A[il])])
    end
    left_legs_list=[]
    push!(left_legs_list,[1,2,3,4,5,6])
    for il=1:N
        legs_list_il=[[1,15,7,8],[9,2,16,7,10],[9,3,17,8,11],[12,4,18,10,13],[12,5,19,11,14],[6,20,13,14]]
    ...
    leigs_res=eigs(leftlm,nev=1,v0=Fl[:],tol=max(ep,1e-15))
    λl,Fl=leigs_res[1:2]
    λl=λl[1]
    Fl=reshape(Fl,DA*Dh^2,Dh^2*DA)
    Fl=1/2*(Fl+ctranspose(Fl))
    L=cholfact(Fl)
    ...
    for ir=N:-1:1
        append!(right_tensor_list,[A[ir],T[ir],Tc[ir],T[ir],Tc[ir],conj(A[ir])])
    end
    right_legs_list=[]
    push!(right_legs_list,[1,2,3,4,5,6])
    for ir=1:N
        legs_list_ir=[[15,1,7,8],[9,16,2,7,10],[9,17,3,8,11],[12,18,4,10,13],[12,19,5,11,14],[20,6,13,14]]
    ...
    reigs_res=eigs(rightlm,nev=1,v0=Fr[:],tol=max(ep,1e-15))
    λr,Fr=reigs_res[1:2]
    λr=λr[1]
    ...
    for ir=N:-1:2
        ...
        Rs=diagm(resir[:S][1:chic])*resil[:Vt][1:chic,:]
